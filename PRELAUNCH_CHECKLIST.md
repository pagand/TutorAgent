...
- [x] **Cut over to a Terraform-built instance, and verified it as a student in a real browser (2026-08-09).**
  `terraform apply -replace=aws_instance.app`, reviewed resource-by-resource before applying: exactly four actions, `aws_instance.app` and `aws_eip_association.api` replaced plus the two CloudWatch alarms re-targeted, and **`aws_eip.api` itself untouched**, so the address survived and CloudFront's origin never moved. New instance `i-09c99b4723608676e`, Elastic IP still `32.184.88.152`. Terraform state and reality agree, which the alternative (moving the EIP onto the hand-built rehearsal box) would have permanently broken.
  **Verified through the real production path, not curl:** loaded `https://air.da-tu.ca` in Chrome, logged in with a real token, got "Welcome, Test Student 5" (so the new 5-token roster is live end to end), started the exam, saw the 25-question nav strip and a running 24:54 timer, and clicked Get Hint. A genuine Gemini hint came back tagged `Conceptual` with real Socratic content and no answer leaked - **not** the canned `hint_style: "error"` payload that `hints.py` returns with a 200 when the LLM path is broken. Full chain exercised: browser → CloudFront → Elastic IP → nginx → api → Gemini.
  The token used for this was then reset, so the handover state is `users=0 sessions=0 interactions=0 llm_calls=0` with all five participants `unused`.
  **Rebuilding the box is therefore five steps, not four.** The Terraform plan's four resource actions get the stack running, but they do not tell you the roster arrived. Step five is: open `/admin/students` and confirm the Enrolled count equals the roster you seeded. That check is what caught the defect below, and resetting the box to a clean handover state is exactly the state in which the old code showed nothing at all.
- [x] **P0. The Students tab showed "No users yet" on a box with a fully seeded roster, and that is the one screen you check before an exam opens.**
  **Found 2026-08-09 by the user, on the freshly cut-over box**, immediately after it was reset to a clean handover state - which is precisely the state the bug needs, and precisely the state the box will be in on exam morning.
  `get_all_users_summary` selected `FROM users u`, but a `users` row is only created at **first login**. Five participants were seeded and every token worked, yet the roster was invisible: `users=0 participants=5`. So the proctor's pre-exam question - "are all my students loaded?" - was unanswerable from the admin UI, and the honest-looking answer it gave instead was "No users yet", which reads as *nothing is seeded* rather than *nobody has logged in yet*.
  **Fixed** by driving the query from `participants FULL OUTER JOIN users` rather than `users` alone, with `COALESCE(u.id, p.token)` as the identity. The outer join is deliberate over a plain left join: it keeps a legacy `users` row that has no participant (the dev DB has 55 of them) as well as a seeded participant who has never logged in, so neither side can be silently dropped. A new `has_user` flag distinguishes them, and the Research tab filters on it, because a roster entry with no login has no preferences and so no A/B group and would otherwise inflate every figure there with students who never sat the exam.
  The `Total Users` tile is renamed **`Enrolled`**, since it now counts the roster rather than the logins.
  Verified against the dev DB, which happens to contain both shapes: 60 rows total, 55 with a `users` row, 5 roster-only, zero duplicates. 152 pytest still pass.
  **Why the earlier verification missed it:** every check of this page had been run against a database that already had users in it. The empty-roster case only appears on a box that is clean - which is every box on exam morning.
- [x] **Scope of the restore bundle, decided and implemented.**
  **First, a correction to this document's own previous claim that no snapshot mechanism existed.** It does: `scripts/snapshot.sh` and `scripts/snapshot_restore.sh`, with two bundles already sitting in `s3://aitutor-backups-434195712367/snapshots/` from 2026-08-05. The earlier "nothing to remove" reading came from checking EC2 snapshots and AMIs, of which there are genuinely none, and never looking in the backups bucket. Same failure shape as the rest of this document: the right check aimed at the wrong place.
  **What it captured, before this change:** Postgres dump, the `chroma_data` volume, container logs, a per-table CSV export of all 9 tables, and a point-in-time copy of `prod/data`. Five things, of which two rebuild themselves.
  **Rescoped to what cannot be rebuilt**, applying one test to each item - can a fresh box regenerate this? The CSV export is derivable from `dump/` with `pg_restore` and `\copy`, and `prod/data` is re-synced from the ops bucket by `ec2-bootstrap.sh` on every single boot, so both were padding the artifact you reach for in a recovery with copies of things that recover themselves. Both removed.
  **What stays, and why each one earns its place:** the Postgres dump, because it is the only genuinely irreplaceable state and **restoring it on top of an already-booted box is the case that was tested** - the dump from the live box was applied with `pg_restore --clean --if-exists` into a separately built instance that had booted from IaC and never seen that data, and it took on the source box's exact state while continuing to serve. The `chroma_data` volume, because `llm_cache.db` lives inside it (`app/services/rag_agent.py:149`) and rebuilding that cache costs real Gemini calls - the Chroma index itself re-ingests in about two seconds and is only along for the ride. And the container logs, which fail the same test the CSVs passed: they die with the container and nothing else records them, which is the open P1 in section E.
  **One risk this keeps, stated rather than hidden:** F5 records that a `chromadb` version bump can make an archived index fail to load. `snapshot_restore.sh` must therefore never treat a failed `chroma_data` restore as fatal, since the index is regenerable and the dump beside it is not.
- [x] **A from-scratch instance boots to a fully working stack in 3m58s with zero manual intervention, proven side-by-side against the live box (2026-08-09).**
  This is the rehearsal the open "destroy plus re-apply" P0 in section G kept asking for, run in the safe direction: a **second** instance was launched alongside the live one rather than replacing it, so there was never a moment with no working box.
  `i-048d5e972b708233d`, tagged `Name=aitutor-rehearsal` **deliberately not** `aitutor-app`, because `scripts/update-prod-data.sh` selects its target with `--filters Name=tag:Name,Values=aitutor-app` and takes `Reservations[0]`, so a second box carrying that name would make the roster-refresh script target an arbitrary one of the two.
  Launched from the same pinned AMI, subnet, security group and instance profile as `terraform/compute.tf`, with `user_data.sh.tftpl` rendered against the real tfvars.
  **Everything came up unattended:** dnf and docker install, 2GB swap, deploy key from SSM, repo clone, `.env` render, prod-data sync from S3, CloudFront origin-range refresh, `docker compose up --build`, wait-for-healthy, participant seed. `api` and `db` healthy, nginx up, `alembic current` at `e4b7c05f1a68 (head)`, 25 questions loaded, 21 RAG chunks ingested, `LLM_PROVIDER=google`.
  **Verified with real LLM calls, not status codes:** a genuine hint returned real Gemini content with `hint_style: "Analogy"` in 1265ms and a chat turn returned a real guiding response in 724ms - specifically checking the body, because `hints.py` degrades to a canned `hint_style: "error"` payload on failure and still returns 200, which is exactly how the `LLM_PROVIDER=ollama` bug hid for a whole stage.
  **The side-by-side is what makes this evidence rather than assertion.** The same read-only checks on the live box returned the *old* three-tile Students page with no `active` column, against the new box's five tiles and the column, and the old roster against the new one. The two boxes differ in precisely the ways the new commit predicts and in no others.
  **Answer to "what has to be done on top of the IaC": nothing.** No intervention was needed on the instance itself. The four frictions found were all in *inspecting* the box, not running it, and belong in the runbook's SSM section rather than in any script: SSM runs as root and needs `git config --global --add safe.directory /home/ec2-user/AITutorApp` before any git command; psql needs `-U aitutor -d aitutor_db`; `/usr/bin/time` does not exist on AL2023; and driving a real request through nginx needs **both** `X-Origin-Secret` and `X-API-Key` from SSM Parameter Store, since one alone gets a 401.
- [x] **Correction: "the box has 3 participants" was wrong, and `git status` on a booted box is never clean.**
  Both found during the rehearsal above, both previously recorded as fact in this document.
  The live box's `participants` table holds **5** rows (`Z7XN5Z4H`, `HYDTEBFX`, `3XMFN8TA`, `ZMGDGX9P`, `6M6JMPCV`), not the 3 that section J's open "number of participants" row still claims. The 3-row figure came from the *local dev* `participants.csv` and dev DB and was never re-checked against the deployed box, which had diverged.
  Separately, `nginx/cloudfront-origin-ips.conf` is a **tracked** file that `scripts/ec2-bootstrap.sh:106-119` regenerates in place from the live AWS ip-ranges feed on every boot, so a healthy booted box always reports `M nginx/cloudfront-origin-ips.conf`. It is self-consistent (the boot-time `git reset --hard` runs before the refresh, not after), but it means **"is the working tree clean?" can never be used as a deploy-health signal on this box**, and any future check that assumes it can will produce a false alarm.
...
- [x] **P0. The self-heal is NOT yet proven end to end.**
  **Proven on the real box, 2026-08-09.**
  The test needed a way to force `api` back with a genuinely different bridge IP, since a lone restart normally reclaims the same IP and would prove nothing.
  Method: baseline `/health` through nginx returned 200.
  `api` was stopped and a throwaway `alpine` container was parked on the freed bridge IP so `api` could not get it back.
  `api` came back on a different IP, `172.18.0.2` -> `172.18.0.5`, confirmed ready by curling its new IP directly, bypassing nginx, so app boot time was not confused with nginx resolution time.
  Through nginx, `/health` returned **502 at t+0** (proof the cached entry really was stale and the test had teeth) and **200 at t+2s**.
  The nginx container's `StartedAt` was identical before and after, so it self-healed rather than being restarted.
  The squatter container was removed and `/health` returned 200. The `resolver 127.0.0.11 valid=10s` fix from the row above is verified, not just deployed.
...
- [x] **P0. The admin dashboard took minutes to load and was unusable during an exam. Root cause found in the browser, not the server.**
  **It was never the database.** One page load is **132 network requests**, ~85 of them individual unbundled ES modules, because that is how Streamlit ships its frontend.
  The SSM tunnel serializes concurrent connections - measured at **6 parallel requests taking 2.1s against 0.39s** for the same bytes fetched serially - so 132 serialized round trips is the multi-minute load.
  Streamlit also sends `Cache-Control: no-cache` on `/` and no caching headers on its assets, so all ~85 files were re-fetched on **every** load, which is why it never got faster after the first visit.
  **Why this took so long to find, and the lesson:** every server-side measurement looked perfectly healthy - `curl` returned 200 in 27ms, `oom_kill 0`, peak memory 171MB of 384MB, 0.34% CPU, zero restarts, Postgres answering instantly. `curl` makes **one** request, so it could not reproduce a defect that only exists at 132. This is CLAUDE.md 0.1 again in a new costume: the same class of miss as the CSP blank page, where every gate was curl-based. Three wrong root causes (connection pool, restart loop, tunnel bandwidth) were proposed and deployed before anyone opened the browser's network tab.
  **Fixed** (`d76a7e1`): nginx now fronts Streamlit on 8501 with gzip and `immutable` caching for `/static/`, whose filenames Streamlit already content-hashes. The published port moved from the streamlit service to nginx, so the documented tunnel command is unchanged and streamlit is no longer reachable from the host at all.
  **Measured after:** 242KB bundle → 74.8KB gzipped (3.2x), 132 requests → 58, and the dashboard renders with live data immediately instead of hanging.
- [x] **P1. Admin *actions* (e.g. Reset User Progress) reportedly do nothing.** Resolved by retiring Streamlit entirely (`3bb2c1f`).
  The gzip and caching fix above made the page *appear* faster and fixed nothing else: selecting a user still took minutes and actions still silently did nothing.
  Re-measured layer by layer, and every layer except Streamlit was clean: Postgres 62ms for the full user-detail query set, the SSM tunnel 640 KB/s at 21ms RTT, nginx bypassed behaving identically, zero Postgres locks during a hang.
  Streamlit sat at **0.36% CPU while the UI showed "running"**. The server was not slow, it was doing nothing.
  Two distinct failures reproduced: a **permanent session wedge** that a full browser refresh does not clear (only `docker compose restart streamlit` did), and a **silent death loop**, 5 deaths in 12 minutes with no traceback, from a reconnect storm walking memory into the cgroup cap.
  Both follow from the same architectural choice, a stateful websocket-only rerun-the-whole-script UI with no HTTP fallback, so it was replaced rather than tuned. See `docs/ADMIN_UI_PORT_PLAN.md`.
  The replacement is a stateless FastAPI router at `/admin` in the existing api container: plain GETs, POST-Redirect-GET for writes, no websocket, no server-held session, so a refresh always recovers. Same tunnel command, same loopback-only exposure, no new service.
  Two things it fixes rather than reproduces: destructive actions now require typing the exact user id (Streamlit had no confirmation, flagged as a risk in the runbook), and POSTs are Origin-checked, restoring the CSRF protection Streamlit's XSRF gave us. The tunnel makes `localhost:8501` reachable from any tab the proctor has open, so that mattered.
- [ ] **P1. Superseded, kept for the measurements. Original entry below.**
  Reported during the bug bash on `HYDTEBFX`: the action appeared to do nothing after a long wait.
  The Streamlit container log contains **no exception at all**, so nothing raised server-side - consistent with the request never completing over a websocket that was starved by the 132-request storm above, rather than with a bug in the action itself.
  The load fix plausibly resolves this as a side effect, but that is an inference, not a measurement. **Re-test explicitly** before exam day.
...
**All three were closed on 2026-08-05.** Kept here as a record rather than deleted, since two of them turned out to matter more than the one-line summaries suggested.

- [x] **P0. Cost allocation tag verified, and the guardrail is now proven rather than merely configured.** `aws ce list-cost-allocation-tags` shows `Project` = `Active`, and `aitutor-monthly-budget` now reports **$0.182 actual against the $15 limit**, up from the $0.00 it had reported indefinitely. A budget that reports real spend is the whole point; until this read non-zero the $15 ceiling was decorative. No backfill was needed.
- [x] **P0. Pushed.** `stage3-hardening` pushed and fast-forwarded into `main`, so the CSP fix is on the remote and a fresh clone plus `terraform apply` can no longer revert the live CSP to the blank-page state.
- [x] **P0. Bug bash triaged and fixed.** The reported symptoms (a new token bounced to the previous student's results, "Failed to submit. Please try again." until a manual refresh, results reading as zero answered, an unusably slow admin dashboard) resolved to four distinct defects, all fixed and guarded: the `QuizProvider` never being reset on logout combined with an init guard matching only `userId` (section 0), the unscoped `examResults` localStorage key leaking one student's results and answer key to the next (section 0), `POST /session/logout` locking a student out of their own exam for 60 seconds (section 0), and a cartesian-product join making the Streamlit dashboard quadratic in student activity (section E). Commits `d259923`, `b770ed5`, `2b511eb`.

**Open P0s consolidated here, detail lives at their original location:**

- [x] ~~EBS snapshot before and after each exam~~ → `scripts/backup.sh` after each exam, proven restorable into a fresh box, section A.
- [ ] Full dress rehearsal on the real AWS stack plus k6 at 50 VUs against real Gemini, section F.
- [x] Instance rebuild proven, section G. `terraform apply -replace=aws_instance.app` rebuilt the box from scratch on a fresh root volume in 2m52s and came back fully healthy. A full-stack `terraform destroy` is still not proven and is now tracked as a separate, lower-value item, since it would tear down CloudFront and ACM and take the live domain dark.
...
| [x] | **P1** | No non-destructive per-student repair | **Fixed.** Implemented as "Unlock question" on the per-student Manage tab of the admin UI. It stamps a new nullable `interaction_logs.voided_at` (migration `c81f4e2a9d33`) rather than deleting, so the voided rows stay in the CSV export and the research record still shows what happened and that an admin intervened. Voided rows are excluded in exactly two places, both required: the attempt cap in `app/endpoints/answer.py` and the interaction history in `app/state_manager.py` that the frontend rebuilds question state from. Excluding only the first would have made the repair cosmetic - the server would accept a new attempt while the student's UI stayed locked, the same trap the bulk timer extension hit in Stage 2 (section 0 above). Two operational caveats recorded rather than hidden: the student must reload, nothing pushes this to an open tab the way a timer change does; and it is the remedy for the system having locked someone wrongly, not a way to grant a second attempt, since at 2 failed attempts the answer key for that question has already been sent to that student's browser. Guarded by `tests/test_answer.py` (voiding restores a locked question to accepting submissions, and the voided rows still exist in the DB) and `tests/test_admin_ui.py` (CSRF still covers the unlock route with a data-unchanged assertion). |
...
| [x] | **P0** | ~~EBS snapshot before and after each exam~~ → **`scripts/backup.sh` (pg_dump to S3), and it is now proven restorable end to end** | **Reframed and closed 2026-08-09, after measuring what a snapshot would actually be protecting.** The original row assumed the machine needed preserving. It does not: a from-scratch IaC boot rebuilds the OS, the code, the schema, the questions, the participants, the RAG index and the secrets in 3m58s with no human input (see the rehearsal in the 2026-08-09 section). **The only thing on that box that no script can rebuild is the student data**, and a full dump of a live DB is **52 KB**. An EBS snapshot is 30 GB of machine to protect 52 KB of data, and worse, it faithfully preserves any hand-patched drift on the box, which is precisely what this project's IaC discipline exists to prevent. **Proven, not argued:** `scripts/backup.sh` was run on the live box (52 KB dump, uploaded to `s3://aitutor-backups-434195712367/aitutor-backups/`), then restored with `pg_restore --clean --if-exists` into the *separately built, never-connected* rehearsal instance. That box went from its own fresh state (`participants=5 users=1 interactions=0 chats=1`) to the live box's exact state (`participants=5 users=5 interactions=108 chats=8`, the live tokens), and kept serving `200` on `/admin/students` afterwards. **So the recovery story is: a fresh box from Terraform plus a 52 KB dump reconstitutes everything, and neither half depends on the dead machine.** Exam-day sequence is therefore `scripts/backup.sh` immediately after the exam window closes, before the instance is stopped, and the restore command above is the recovery path. A pre-exam snapshot is still defensible as a fast mid-exam rollback, but it is a convenience, not the durability mechanism, and it is no longer a P0. |
...
| [x] | **P1** | One-command instance snapshot (`scripts/snapshot.sh`) | **Built.** Requirements and implementation plan are in "Instance snapshot: requirements and plan" immediately below this table; the plan is now implemented as specified there. `scripts/snapshot.sh` captures everything associated with the instance (Postgres via `backup.sh`, the `chroma_data` volume including `llm_cache.db`, `prod/data`, all four services' container logs) plus a CSV export of all 9 tables and a data-dictionary `README.md` (including the corrected `user_action_logs.action_type` values, see the known-defect note below), bundles it under one timestamped key (`snapshots/aitutor_snapshot_<ts>.tar.gz`) in the backups bucket, and does not touch the ops bucket. Not a cron. Restore direction is a new script, `scripts/snapshot_restore.sh` (not an extension of `restore.sh`, kept separate so `restore.sh`'s single-purpose, explicit-target safety property stays easy to audit); it downloads/unpacks the bundle, calls `restore.sh` unchanged for Postgres, and restores the `chroma_data` volume via a scratch container, briefly stopping `api` for both the capture and the restore. Closes the §9 research-data-export gap in `docs/OPS_RUNBOOK.html`, including the `skill_mastery` bulk export that had no path before. **Tested 2026-08-05:** the Postgres capture-restore round trip (via `backup.sh`/`restore.sh`'s non-Docker code path) and the CSV export were run against seeded scratch databases (`aitutor_snaptest_src`/`_dst`, migrated with `alembic upgrade head`, since dropped), all 9 tables matched row-for-row after restore, and all 9 CSVs produced real rows with the documented JSON-column behavior confirmed. `bash -n` and dry runs (missing-argument / missing-env-var / nonexistent-bundle-path failure paths) passed on both scripts. **The `chroma_data` Docker-volume capture and restore steps are UNTESTED**, they cannot be exercised locally (no local Docker Compose stack per this repo's dev-parity rule) and have not yet been run against the real box. Also unverified: the actual S3 upload/download path and the `docker compose exec`/`docker inspect` invocations, which need the real Compose stack. Treat the volume and S3 legs as reviewed-but-unproven until run for real. **Superseded 2026-08-05 by two live runs against the real box, see the row below.** |
| [x] | **P1** | Snapshot proven on real infrastructure, restore proven for Postgres, one leg still unproven | **Run live 2026-08-05, twice: once against a freshly bootstrapped box and once against a box holding real exam data from two students who took the exam through `https://air.da-tu.ca`.** This closes the three legs the row above listed as unproven: the `chroma_data` Docker-volume capture, the real S3 upload, and the `docker compose exec`/`docker inspect` invocations all worked. Second bundle: `snapshots/aitutor_snapshot_20260805T233805Z.tar.gz`, 491,122 bytes, against 455,909 for the empty-box run. `api` was confirmed healthy after the deliberate stop/start on both runs, and ChromaDB reported the same `Collection count: 21` afterwards, so the volume capture is not corrupting. **Capture fidelity:** all 9 CSVs matched the live database exactly (`users` 2, `participants` 3, `exam_sessions` 2, `interaction_logs` 23, `skill_mastery` 5, `user_action_logs` 111, `chat_logs` 3, `intervention_logs` 0, `llm_usage_log` 7), including per-student detail, Alice 19 interaction rows and 2 chat rows with a non-null `submitted_at`, Bob 4 and 1 with a null one, so the completed-versus-in-progress distinction survives. **Restore:** the bundle was pulled fresh from S3 and its dump restored into a scratch database via the identical `restore.sh` call `snapshot_restore.sh` makes internally; all 9 tables matched row-for-row. The `chroma_data` tar was extracted and inspected separately: `chroma.sqlite3` 643,072 bytes, `llm_cache.db` 147,456 bytes, and all four HNSW index files present and non-zero. **STILL UNPROVEN, stated plainly:** `snapshot_restore.sh` has never been executed as a script. Its `chroma_data` restore step unconditionally resolves the volume mounted into the live `api` container and deletes its contents before untarring, with no scratch-volume option, so running it would have destroyed the real vector store. Its own bundle download, extraction and dump discovery were replicated by hand rather than executed. Proving the script faithfully needs a genuinely second box or a throwaway volume, and is the one remaining gap in the recovery path. |
...
#### KNOWN DEFECT, now fixed: the `UserActionLog` docstring was stale

**Fixed in `app/models/user.py`.** The docstring now lists the 19 action types the frontend actually emits (the same list below), and explicitly warns that the validation whitelist in `app/endpoints/action_log.py` is a different thing and is correct as it stands, holding these canonical names AND keeping the old ones as explicit legacy aliases, so it must NOT be "fixed" to match this list. The rest of this block is kept as the record of what was wrong and how it was found.

`app/models/user.py:120-132`'s docstring listed action types the application never emits, and omitted several it does.
...
| [x] | **P1** | Graceful LLM failure handling | **Found stale during this pass and closed as already-done, not as new work.** Verified directly against the code: `frontend/src/components/QuizPageContent.tsx` (`llmErrorMessage`) and `frontend/src/components/ChatPanel.tsx` (`chatErrorMessage`) both map a 429 or 5xx from the hint/chat endpoints to "The tutor/hint is temporarily unavailable. Please try again in a moment.", preferring the 429's own `detail` string (the per-user/global LLM cap's specific message) when present. `app/endpoints/hints.py` and `app/endpoints/chat.py` both wrap their bodies in `except Exception` and return a clean `HTTPException(status_code=500, ...)` rather than leaking the exception. This row and its costing (**`docs/OPS_RUNBOOK.html` §11, recommendation 5**) had gone stale: the runbook's own incident-response table (§8) now records this as "Handled gracefully, not an open gap" and recommendation 5 has been removed from the §11 table, both corrected to match this finding. |
| [x] | **P2** | Admin-UI toggle for the LLM spend caps | **Fixed.** DB-backed (`admin_settings` table, migration `e4b7c05f1a68`) rather than in-process, deliberately: process state would silently revert on an `api` restart, and a raised cap quietly dropping back mid-exam would re-throttle students with nobody noticing. `app/services/llm_quota.py`'s `reserve_llm_call` reads the override via `get_effective_caps` with a fallback to `.env`-sourced `settings`, and a malformed override value is ignored with a `logger.warning` rather than raised, because that code path runs on every hint and chat request and a bad settings row must never be able to take the tutor down. Admin UI form lives on the Exam tab (`POST /admin/settings/llm-caps`, `POST /admin/settings/llm-caps/clear`). Guarded by `tests/test_admin_ui.py` (round trip through the override and back to the `.env` default, the malformed-value case, and CSRF coverage with a data-unchanged assertion). |
...
| [ ] | **P2** | Hints and chat render raw Markdown and LaTeX instead of formatted text | **Found 2026-08-05 while exercising the live deployment as a real student.** Hint text and tutor chat replies display the model's literal source: `**Step 6: State the Result**`, `*previous velocity*`, and `$v_t = 3.1$` all appear with the asterisks and dollar signs visible. Reproduced consistently across multiple hints and chat turns for two different students, so it is systemic, not a one-off. It matters more here than it would elsewhere because the subject is deep learning optimization, so the maths is the content: `$v_t = 3.1$` is noise to a student reading under a 25 minute clock. Two options, neither started. The thorough fix is a Markdown renderer with maths support (`react-markdown` plus `remark-math`/`rehype-katex`), which means new dependencies, added bundle weight, and care with KaTeX's font loading under the frontend's strict CSP (`script-src`/`font-src` are locked down, see section D). The cheap fix is prompt-side, instructing the model in `app/services/prompt_library.py` and `app/endpoints/chat.py` to emit plain text, which costs nothing but is unreliable since models drift back to Markdown. **Deferred by the user 2026-08-05 to look at later.** |
...
The gap that remains is failure *alerting*, not behavioural data or request-level failure *visibility* - the three rows above closed that half.

**`UserActionLog` docstring, referenced from this section's "already good" claim above:** was stale (see the KNOWN DEFECT block under section A), and is now fixed. See that block for detail; not repeated here.
...
| [ ] | **P1** | k6 load test that hits Gemini for real | Phase 3.6 specifies 50 VUs. It must make real LLM calls, otherwise it validates nothing about the actual failure mode. Pass criteria: p95 < 5s non-LLM, < 90s for hints and chat. **No script exists yet**, this row names the test but nothing implements it. |
| [ ] | **P1** | Ramp the load test rather than only running a flat 50 | A single 50-VU pass/fail answers "did it survive", not "where does it break", and a pass at 50 tells us nothing about the margin. Run a staircase, **1, 2, 3, 5, 10, 20, 35, 50 VUs**, holding each step long enough for latency to settle, and record p50/p95 **and** `docker stats` CPU and memory per step. What matters is the shape: latency should stay flat while CPU climbs linearly, and the step where p95 starts bending upward is the real capacity, which may be well below or above 50. If the knee is at 50 there is no margin and a single slow student pushes it over. Two specific suspects to watch, both already documented in section H: the mandatory single uvicorn worker, and the 6-slot Chroma executor with synchronous embedding calls. |
| [ ] | **P1** | Decide what happens if the ramp finds the knee below 50 | Worth deciding *before* the number is known, so the answer is not chosen under time pressure on exam day. The options are a bigger instance, staggered student start times, or capping concurrent sessions, and they have very different lead times: resizing EC2 is minutes, staggering starts is a conversation with whoever runs the exam. |
...
| [x] | **P0** | **The unpinned AMI would have destroyed the box on any future `terraform apply`** | **Found and fixed 2026-08-05, and it is the most dangerous thing in this document's history that never actually fired.** `data.aws_ami.al2023` used `most_recent = true`, so the resolved AMI id changed every time AWS published a new AL2023 image. `ami` forces replacement on `aws_instance`, and the root volume carries the `postgres_data` and `chroma_data` Docker volumes, so a replacement destroys the database, the vector store and the LLM cache. **Caught only because a DNS-only change was planned rather than applied blind:** `terraform plan` for `enable_custom_domain = true` came back `3 to add, 5 to change, 2 to destroy` with `instance_id -> (known after apply)`, and `replace_paths` named `["ami"]`, `ami-06503266fb468d937 -> ami-072f44400e27b9115`. Nothing about the DNS change touched compute. This was armed on **any** apply, for any reason, by anyone, with no warning beyond a plan nobody was required to read closely. **Fixed** by replacing the data source with a pinned `var.ami_id` defaulting to `ami-06503266fb468d937`, the image the live instance was actually built from, so it was a no-op against state at the time of pinning. Bumping it is now an explicit reviewed decision: change the value, confirm the new image's root snapshot still fits `root_volume_gb`, and expect a full replacement. This closes the AMI layer of F5, the row below; the other four layers remain open. |
| [ ] | **P1** | F5 — nothing is version-pinned, so a rebuild is not reproducible | **Partially closed 2026-08-05: the AMI layer is now pinned, see the row above, and it turned out to be a data-loss risk rather than a reproducibility nicety. The remaining four layers stay open.** **Originally deferred, Stage 5** (see the Stage 5 plan's "out of scope" section). Five independently floating layers: the AMI (`most_recent = true`, already bit once — its 30GB root snapshot forced `root_volume_gb` up from the planned 20GB), `repo_ref = "main"` rather than a tagged commit, Python deps on range constraints with no lockfile (`chromadb`'s on-disk index format is the specific risk — a version bump can make an archived `chroma_data` volume fail to load), floating base image tags (`python:3.11-slim`, `postgres:16-alpine`, `nginx:alpine`), and — moot since Stage 5 deleted certbot — its `:latest` tag. Deliberately left open: fixing it now would mean editing `terraform/templates/user_data.sh.tftpl`, and any edit there forces a full instance replacement (`user_data_replace_on_change = true`), which is exactly the kind of unnecessary risk this stage was trying to avoid while other changes were still landing. |
...
| [ ] | **P0** | **Stage 4's own gate, a `destroy` plus re-apply reproduces the box identically, was never performed** | **Restored as an explicitly tracked open item, found during the 2026-08-04 audit.** This was Stage 4's own stated gate (see Execution sequence above), and it was quietly dropped, it does not appear as open anywhere in this document until now. The current box has accumulated several fixes as one-off live interventions rather than through `user_data.sh.tftpl` or Terraform: the buildx plugin install, the root volume size correction, two SIGPIPE-under-pipefail bugs, a `.dockerignore` gap, and a manual cleanup of a leftover `certbot/` directory (all logged under Stage 5 above). None of these were proven to survive a `destroy` and a from-scratch re-apply. Until this gate actually runs, the box's working state should be treated as live-patched, not reproducible. **Substantially addressed 2026-08-05, though not by the literal `destroy` this row names.** `terraform apply -replace=aws_instance.app` was run against the live stack, which destroys the instance and its root volume and rebuilds it from `user_data.sh.tftpl` on a genuinely fresh disk. That is the part of the gate that actually proves the bootstrap is reproducible; the live-patched fixes this row worried about either survived into the repo or were never needed. Result: new instance `i-068fb380c5ec1d2d4`, cold bootstrap 2m52s, all four containers up, `LLM_PROVIDER=google` confirmed inside the api container, 3 participants seeded, 25 questions across 5 skills loaded, RAG ingest of 7 pages into 21 chunks, and `https://air.da-tu.ca/api/` serving through CloudFront. The Elastic IP survived unchanged (`prevent_destroy` plus a recreated association), so the CloudFront origin never moved. **What is still NOT proven:** a full `terraform destroy` of the whole stack. That would tear down CloudFront and ACM, take `air.da-tu.ca` dark, mint a new distribution domain, and fail on the Elastic IP's `prevent_destroy` guard unless it is deliberately removed. Left open as a distinct, lower-value exercise now that the compute bootstrap itself is proven. |
...
| [x] | **P2** | The `streamlit` container reported `unhealthy` permanently, and the runbook said it had no healthcheck at all | **Found and fixed 2026-08-05**, during the first-boot verification of the rebuilt instance. `docker compose ps` showed `aitutorapp-streamlit-1` as `Up (unhealthy)` while Streamlit was serving fine. Cause: `streamlit` and `api` build from the same root `Dockerfile`, which bakes in `HEALTHCHECK CMD ... urlopen('http://localhost:8000/')`. The compose file overrides the command so this container serves on 8501, and nothing listens on 8000 inside it, so the inherited probe failed forever with `OSError: [Errno 99]`. Longstanding, dating to the Phase 2 commit `9cdf238`, not introduced by any recent change. Two consequences: a permanently red container in the one command an operator runs to check system state, which trains people to ignore it, and `docs/OPS_RUNBOOK.html` asserting the opposite ("nginx and streamlit have no healthcheck defined"), so the runbook could not have explained what an operator was looking at. **Fixed** with a streamlit-specific `healthcheck` in `docker-compose.yml` against `http://localhost:8501/_stcore/health`, Streamlit's own health endpoint. Runbook corrected in the same pass: streamlit now has a healthcheck, nginx genuinely has none. |
...
| [ ] | Number of participants per exam | Confirm the real cohort size. `prod/data/participants.csv` now holds **5** rows (`tst_001`-`tst_005`, minted 2026-08-09 for the manager walkthrough, spread across all four group/intervention combinations); every cost and capacity figure in this document assumes 50. The earlier "3 rows" reading of this row was wrong about the deployed box, see the correction in the 2026-08-09 section. |
...
**Recounted mechanically this pass** (parsed by grepping every `| [ ] | **P0/P1/P2** |` / `| [x] | **P0/P1/P2** |` table row in this file, not tallied by hand, and not reconciled arithmetically against the stale 2026-08-04 numbers below - the file has grown by many rows across several sessions since then, so this is a fresh count against current content):

| Priority | Open | Done | Total | Meaning |
|---|---|---|---|---|
| P0 | **3** | 40 | 43 | Blocks go-live |
| P1 | **16** | 26 | 42 | Should be fixed before go-live |
| P2 | **16** | 6 | 22 | Post-exam cleanup |
| **Total actionable** | **35** | **72** | **107** | Every actionable item carries a priority |

**What moved this pass:** six table rows closed, all P1 → done or P2 → done, no new rows added and no re-prioritizations. P1 (5 closed): "No non-destructive per-student repair" (section 0, the admin "Unlock question" repair), "Graceful LLM failure handling" (section B, found already-implemented and closed as stale rather than as new work), and the three-row observability set "Request logging middleware" / "Structured (JSON) logging" / "Latency metrics" (section E). P2 (1 closed): "Admin-UI toggle for the LLM spend caps" (section B). P0 is unchanged at the table-row level (3 open, unchanged), but the single most significant closure this pass is **not** a table row: the top-of-document bullet "The self-heal is NOT yet proven end to end" is now proven and marked `[x]` - it was tracked inline at the top of the document rather than as a section table row, so it does not move the mechanical P0 tally above, but it was the last unverified claim behind the nginx self-heal fix and is arguably the most consequential item closed this pass.

**Numbers below this line predate the current pass and are kept as history, not corrected in place** - see the fresh count above for current state.
...
One caveat on that last point applied at the time this was written: the `UserActionLog` docstring names were stale.
It documented `intervention_offered/accepted/rejected`, `session_complete` and `timer_expired`, while the app actually emits `intervention_offer/accept/reject`, `session_submit` and `session_expire`.
Anyone writing analysis queries from the docstring got empty result sets.
**This is now fixed** - see the KNOWN DEFECT block above, which records the corrected docstring content.
