...
**Services needed:** EC2 (stopped except exams), EBS **20GB** gp3 (~$1.60/mo, bills while stopped, reduced from the original 30GB estimate; see Stage 4 note above), Elastic IP, S3, CloudFront (1TB free tier).
...
| [x] | **P1** | Certificate renewal while EC2 is stopped | **Fixed (Stage 4).** `terraform/guardrails.tf`'s `aws_scheduler_schedule.monthly_start`/`monthly_stop` boot the box for 30 minutes on the 1st of each month. `scripts/ec2-bootstrap.sh` runs on every boot and issues-or-renews the cert via certbot in webroot mode before switching nginx to the TLS config, so a renewal happens even if no exam falls in that 90-day window. |
...
|---|---|---|---|---|
| P0 | **5** | 32 | 37 | Blocks go-live |
| P1 | **19** | 15 | 34 | Should be fixed before go-live |
| P2 | **10** | 4 | 14 | Post-exam cleanup |
| **Total actionable** | **34** | **51** | **85** | Every actionable item carries a priority |

**The 5 remaining open P0s are all infrastructure/runbook, not application security.** EBS snapshot cadence, the t3.small→t3.medium resize, `ALLOWED_ORIGIN` pointed at the real CloudFront domain, HTTPS via certbot, and the full dress rehearsal — every one of them needs a real deploy to close (Stage 5/6), none of them can be closed by more code. The application-layer P0 block that motivated Stage 4.5 (unauthenticated access, no LLM cost ceiling, public question paper, closed-state UX) plus the two perceived-performance P0s in section C (UX latency budget, RAG cold start) are all fixed and closed this stage.
