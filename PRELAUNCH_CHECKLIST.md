...
- [ ] **P2. Two small things the browser pass surfaced, neither blocking.**
  `/login` offered **"Continue Session: HYDTEBFX"** from `localStorage` - a token from the old roster that the new box has never heard of. On a shared exam machine a student can be offered a resume for a token the backend cannot honour. `clearLocalSession()` handles the deliberate logout path; nothing handles "the server no longer knows this token", so the probe should treat an unknown token as a cleared session.
  Separately, the token field's placeholder is `e.g., Z7XN5Z4H`, which is a **real token from the previous roster**, not a fabricated example. It is dead now, but a placeholder that could ever match a live credential should be an obviously-fake string.
- [x] **Scope of the restore bundle, decided: it is the `pg_dump`, and nothing else.**
  The question was whether to rescope the abandoned whole-machine snapshot down to "everything not rebuildable", including the RAG store and the LLM cache. Measured rather than assumed, the answer is that the list has one item on it.
  `postgres_data` is the only genuinely irreplaceable state, it is 52 KB, and **restoring it on top of an already-booted box is the case that was tested** - the dump from the live box was applied with `pg_restore --clean --if-exists` into a separately built instance that had booted from IaC and never seen that data, and it took on the source box's exact state while continuing to serve.
  The `chroma_data` volume holds two very different things and must not be treated as one. The Chroma index itself is re-ingested from `prod/data/source.pdf` in about two seconds at every boot, so archiving it buys nothing - and it actively **costs** something, because F5 records that a `chromadb` version bump can make an archived index fail to load, which would introduce a restore-time failure mode that does not otherwise exist. The LLM cache is a single SQLite file at `/app/chroma_db/llm_cache.db` (`app/services/rag_agent.py:149`); it rebuilds itself on demand, so preserving it is a pure cost optimisation with no correctness value, and section C already records that its hit rate on this workload is expected to be low.
  **Therefore:** the bundle is the `pg_dump`. If `llm_cache.db` is ever added it goes in as a separate optional file, never as a precondition, so that failing to restore a cache can never block recovering an exam.
